...
import com.app.whosnextapp.feedpost.TabFeedGroupVideoAdapter;
import com.app.whosnextapp.feedpost.TabFeedGroupVideoViewHolder;
...
public class FeaturedProfileGroupViewHolder extends RecyclerView.ViewHolder implements ToroPlayer, ToroPlayer.EventListener, TabFeedGroupVideoAdapter.OnGroupVideoClickListener, View.OnClickListener {
    ...
    private FeaturedProfileGroupVideoAdapter featuredProfileGroupVideoAdapter;
    ...
    public FeaturedProfileGroupViewHolder(Context context, View itemView, PressablePlayerSelector selector, FeaturedProfileAdapter.OnItemClick onItemClick) {
        super(itemView);
    ...
    private void setSelector(int pos) {
        for (int i = 0; i < groupVideoModels.size(); i++) {
            if (groupVideoModels.get(i).isSelected) {
                ...
                featuredProfileGroupVideoAdapter.notifyItemChanged(i);
            }
        }
        groupVideoModels.get(pos).isSelected = true;
        featuredProfileGroupVideoAdapter.notifyItemChanged(pos);
    }

    @Override
    public void play() {
        if (helper != null) helper.play();
        setSelector(playerView.getPlayer().getCurrentWindowIndex());
    ...
    public void bind(GetFeaturesProfileModel.DashboardList dashboardList) {
        ...
        groupVideoModels = dashboardList.getGroupVideoList();
        featuredProfileGroupVideoAdapter = new FeaturedProfileGroupVideoAdapter(context, groupVideoModels, this);
        ...
        rv_group_video.setAdapter(featuredProfileGroupVideoAdapter);
